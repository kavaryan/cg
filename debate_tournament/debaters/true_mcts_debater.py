...
class TrueMCTSDebater(BaseDebater):
    ...
    def __init__(self, side: str, motion: str, iterations: int, max_rollout_depth: int, max_debate_depth: int, exploration_constant: float, dry_run: bool = False):
        super().__init__(side, motion)
        self.mcts_algorithm = MCTSAlgorithm(side, motion, iterations=iterations,
            max_rollout_depth=max_rollout_depth, max_debate_depth=max_debate_depth, exploration_constant=exploration_constant, dry_run=dry_run)
